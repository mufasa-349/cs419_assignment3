...
\item Foreground/background separation via Otsu thresholding (or adaptive thresholding for uneven illumination).
...
\[
\text{sure\_fg} = \mathbb{1}\left( D(x,y) > \alpha \cdot \max(D) \right)
\]
...
Invert threshold & Needed if cells are darker in chosen channel \\
...
All code is provided in \texttt{watershed\_cellseg.py}. For batch evaluation:
\begin{verbatim}
