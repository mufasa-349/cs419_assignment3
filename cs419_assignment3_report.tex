...
\begin{enumerate}
\item \textbf{Image normalization:} Convert input images (including 16-bit TIFF) to 8-bit for OpenCV processing via min-max normalization.
\item Convert image to an informative single-channel feature map (default: grayscale; optional: LAB/HSV channels via \texttt{extract\_feature\_channel}).
\item Contrast enhancement using CLAHE (Contrast Limited Adaptive Histogram Equalization) with configurable clip limit and tile size.
...
feat_blur = cv2.GaussianBlur(feat, (5, 5), 0.0)

# Otsu thresholding with auto-invert
if auto_detect_invert(feat_blur):
    _, bw = cv2.threshold(feat_blur, 0, 255,
...
The \texttt{relabel\_and\_filter} function removes regions smaller than \texttt{min\_area} and re-indexes remaining labels to be consecutive (1..K):
\begin{lstlisting}[language=Python]
...
All code is provided in the file \texttt{32417.py}. For batch evaluation:
